  ...
  async function enviarCodigo() {
    ...
    const msg = document.getElementById("mensagemRecEmail");
    const botao = document.getElementById("botao_enviar_email_recuperacao");
    ...
    try {
      ...
      if (res.ok) {
        ...
        document.getElementById(
          "container_enviar_codigo_recuperacao"
        ).style.display = "flex"; // modal do código
      } else {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTimeout(() => {
          ...
          document.getElementById(
            "container_enviar_email_recuperacao"
          ).style.display = "flex"; // volta modal do email
        }, 2000);
      } else {
        msg.innerText = dados.message;
      }
